...
1. Re-run the branch on `PFEM10` with payload `0.16.0` until the top-level result reaches `process_runtime_active_session_validation_bootstrapped`.
   ...
   - Exit criteria: `pnpm android:local-host:embedded-runtime-pod:doctor -- --json` converges to `classification=process_runtime_active_session_validation_bootstrapped` and `runtimeDescribeAfter.recommendedNextSlice=process_runtime_active_session_device_proof`.

2. Capture the next bounded device-proof slice as `process_runtime_active_session_device_proof`.
   - The branch no longer needs a decision about whether plugin/process-model/process-runtime-activation/process-runtime-supervision/process-runtime-observation/process-runtime-recovery/process-runtime-detached-launch/process-runtime-supervisor-loop/bootstrap-validation should exist; those are now settled repo-side.
   - Exit criteria: the bounded active-session-validation contract turns into one real detached active session on-device with observed lease renewal, recovery re-entry, and restart continuity.
...
2. Keep the active-session-validation contract and the device-proof checklist aligned.
   ...
   - Exit criteria: the docs and runtime status surface both describe one coherent active-session-validation model, and the next gap is only device-side proof.
