...
6. Replay hardening now also has verified explicit perturbation, soak, and reinstall-refresh paths.
   ...
   - The new `pnpm android:local-host:embedded-runtime-pod:refresh` wrapper now packages the reinstall perturbation lane as one formal command too: it auto-discovers the Android SDK root when the current shell is missing `ANDROID_HOME`, re-exports the local-host token without persisting it in artifacts, and falls back to `adb install -r -d` when `pnpm android:install` fails on `PFEM10`. The latest April 10, 2026 real-device run succeeded through that fallback path and still kept the five-iteration restart-perturbation soak green afterward.
   - The top-level runtime status now surfaces both `recommendedHardeningCommand` and `recommendedRefreshCommand`, while the captured active-session-device-proof artifact now also surfaces `hardeningCommand` and `refreshCommand`, so both machine-readable and human-readable handoff paths can point directly at the soak wrapper and the reinstall-refresh wrapper instead of longer raw command sequences.
...
- `pnpm android:local-host:embedded-runtime-pod:refresh -- --json`
  - Passed on April 10, 2026 on `PFEM10`.
  - The aggregated summary reports `ok=true`, `androidSdkRoot="/Users/zouxiaoyi/Library/Android/sdk"`, `install.finalMethod="adb_install_debug_apk"`, `install.primary.ok=false`, `install.fallback.used=true`, `install.fallback.ok=true`, `token.tokenRedacted=true`, `passedIterationCount=5`, `failedIterationCount=0`, and `classifications=["process_runtime_active_session_live_proof_captured"]`.
- `pnpm test -- test/apps/android/local-host-embedded-runtime-pod-refresh.test.ts test/apps/android/local-host-embedded-runtime-pod-soak.test.ts test/apps/android/local-host-embedded-runtime-pod-stability.test.ts test/apps/android/local-host-embedded-runtime-pod-doctor.test.ts test/apps/android/local-host-embedded-runtime-browser-lane-smoke.test.ts`
  - Passed on April 10, 2026 after adding the new refresh wrapper, token-redaction path, SDK auto-discovery, and install fallback handling.
  - The targeted suite now covers the formal reinstall-refresh entrypoint, the longer soak entrypoint, the shorter restart-perturbation replay, and the failure path where the adb restart perturbation itself regresses, while keeping the surrounding doctor and browser-lane smoke coverage green.
- `cd apps/android && env ANDROID_HOME="$HOME/Library/Android/sdk" ANDROID_SDK_ROOT="$HOME/Library/Android/sdk" ./gradlew --no-daemon --console=plain :app:testDebugUnitTest --tests ai.openclaw.app.node.PodHandlerTest`
  - Passed on April 10, 2026 after surfacing the soak and refresh commands through the runtime status / artifact layer.
...
- The longer soak replay is now green too, so the next hardening move should keep `pnpm android:local-host:embedded-runtime-pod:soak -- --json` as the default repeatability lane, and use `pnpm android:local-host:embedded-runtime-pod:refresh -- --json` whenever reinstall plus token-bootstrap perturbation is the question.
