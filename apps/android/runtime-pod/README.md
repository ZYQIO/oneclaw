...
The staged pod metadata plus `browser/`, `workspace/`, `toolkit/`, and `runtime/` assets in this directory are now consumed by `pod.browser.describe`, `pod.manifest.describe`, `pod.runtime.describe`, `pod.runtime.execute`, `pod.workspace.scan`, and `pod.workspace.read`.
