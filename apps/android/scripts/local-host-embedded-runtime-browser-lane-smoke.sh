...
What it does:
  ...
  3. Replays pod.runtime.execute for runtime-smoke and tool-brief-inspect
  4. Reads pod.browser.describe before the browser lane starts
  5. Optionally calls pod.browser.auth.start for the allowlisted auth flow
  6. Polls pod.browser.describe until replay state appears on disk
  7. Reads pod.runtime.describe again and writes a focused summary.json
