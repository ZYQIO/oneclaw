...
- `environment`: partial bootstrap. The bounded replay now leaves app-private state, health-report, restart-contract, process-model, activation-contract, and supervision-contract bootstrap evidence under desktop-home, but there is still no detached subprocess or richer observation/recovery loop beyond those artifacts. / `environment`：部分 bootstrap。有边界 replay 现在已经会在 desktop-home 下留下 app 私有 state、health-report、restart-contract、process-model、activation-contract 与 supervision-contract bootstrap 证据，但在这些工件之外仍然没有真正分离的子进程或更丰富的观察/恢复循环。
...
- Status on this branch: `pod.runtime.execute` now hydrates `filesDir/openclaw/embedded-runtime-home/<version>/` with config, logs, state, and work directories, the bounded desktop-home replay writes profile artifacts plus `runtime-smoke-health-report.json`, `runtime-smoke-restart-contract.json`, `runtime-smoke-process-model.json`, `runtime-smoke-activation-contract.json`, and `runtime-smoke-supervision-contract.json` into desktop-home state, and the first packaged tool lane already writes replayable results under `work/`. The next gap is no longer "should we start a process-runtime activation contract at all?" or even "should we bootstrap supervision?" but "how to deepen that bounded supervised session into stronger observation and recovery semantics on-device." / 这条分支的现状：`pod.runtime.execute` 已经会把 `filesDir/openclaw/embedded-runtime-home/<version>/` 水合成带有 config、logs、state 和 work 目录的 runtime home，有边界的 desktop-home replay 现在也会把 profile 产物以及 `runtime-smoke-health-report.json`、`runtime-smoke-restart-contract.json`、`runtime-smoke-process-model.json`、`runtime-smoke-activation-contract.json` 与 `runtime-smoke-supervision-contract.json` 写进 desktop-home state，而第一条打包工具通道已经会把可复跑结果写入 `work/`。因此下一步缺口已经不再是“要不要先开始 process-runtime activation contract”，甚至也不再是“要不要先 bootstrap supervision”，而是“如何把这条有边界的受控 session 深化成设备内更强的观察与恢复语义”。
...
- Status on this branch: one packaged desktop tool lane plus one narrow allowlisted plugin lane are both landed, the last real-device replay already proved the plugin lane on `0.7.0`, and the current `0.10.0` repo-side slice now adds structured process-model plus activation-contract plus supervision-contract bootstrap artifacts. The next gap is therefore no longer "whether to add a plugin lane", "whether to bootstrap runtime activation", or "whether to bootstrap supervision" but how to deepen that bounded session into stronger observation and recovery semantics. / 这条分支的现状：一条打包桌面工具通道和一条窄白名单 plugin lane 都已落地，上一轮真机已经在 `0.7.0` 上证明 plugin lane，而当前 `0.10.0` 的 repo 侧切片又补上了结构化 process-model、activation-contract 与 supervision-contract bootstrap 工件。因此下一步缺口已经不再是“要不要加 plugin lane”“要不要先 bootstrap runtime activation”或“要不要先 bootstrap supervision”，而是如何把这条有边界 session 深化成更强的观察与恢复语义。
...
- Payload `0.10.0` now carries a packaged `desktop/` stage that groups engine, environment, browser, tools, plugins, supervisor manifests, one desktop profile descriptor, and the first allowlisted plugin descriptor into one cohesive APK bundle.
...
- `runtime-smoke` now replays the materialized desktop profile and packaged environment/supervisor manifests into device-local artifacts, and also leaves explicit health-report, restart-contract, process-model, activation-contract, and supervision-contract bootstrap state.
