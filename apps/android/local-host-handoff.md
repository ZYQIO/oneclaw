...
- 同一天深夜又补上了第三条内置 preset：`OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=calculator-home-open-conversion`。它默认目标包是 `com.coloros.calculator`，会点击稳定的 `com.coloros.calculator:id/item_open_conversion`，把 Calculator 首页收成第二条非 Settings 的 repo-side follow-up 基线；如果想每次都从干净首页开始，直接再叠 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`。Later that night, a third built-in preset landed as `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=calculator-home-open-conversion`. It defaults the target package to `com.coloros.calculator`, taps the stable `com.coloros.calculator:id/item_open_conversion`, and turns the Calculator homepage into the second non-Settings repo-side follow-up baseline; when you want every rerun to start from a clean homepage, just add `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`.
