...
1. 先在真机上复跑 `pnpm android:local-host:ui`，把新的默认 in-app UI smoke 补成明确的设备证据。Replay `pnpm android:local-host:ui` on-device first so the new default in-app UI smoke has explicit device proof.
2. 再把 `ui.launchApp + ui.inputText` 做成可重复的跨 app 真机路径，并调查当前 OPPO / ColorOS 的后台冻结风险，再决定下一条跨 app primitive 是 `swipe` 还是更强 selector。Then turn `ui.launchApp + ui.inputText` into a repeatable cross-app device path and investigate the current OPPO / ColorOS background-freeze risk before deciding whether `swipe` or stronger selectors should be the next cross-app primitive.
...
3. 先跑新的 `pnpm android:local-host:ui`，确认它能稳定完成 `ui.launchApp(OpenClaw)`、`ui.tap(Chat)`、`ui.waitForText(chat-ready)`、`ui.tap(editor)`、`ui.inputText(写入/清空草稿)` 和最终 `ui.state`。Run the new `pnpm android:local-host:ui` script first and confirm it can stably complete `ui.launchApp(OpenClaw)`, `ui.tap(Chat)`, `ui.waitForText(chat-ready)`, `ui.tap(editor)`, `ui.inputText` (write/clear draft), and the final `ui.state`.
