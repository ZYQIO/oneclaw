...
As of March 29, 2026, the MVP happy path is working, the streaming gate has direct on-device proof, the first repeatable UI smoke passes, the cross-app probe plus settings-search follow-up already have real-device evidence, the first direct `ui.swipe` proof now exists on the Settings homepage, and the first multi-window sweep still shows no failure within 30 seconds. The active engineering focus has therefore moved from "is short-window cross-app reachability real?" to "which of the now-proven follow-up actions should become the next reusable baseline?" / 截至 2026 年 3 月 29 日，MVP 的成功路径已经跑通，streaming 门槛已有真机直证，第一条可重复 UI smoke 已通过，cross-app probe 连同 settings-search follow-up 也已有真机证据，第一条 `ui.swipe` 设置首页滚动证据也已补齐，而第一条多窗口 sweep 仍然证明 30 秒内没有掉线；当前工程重心因此已经从“短窗口跨 app 可达性是不是真的”转到“已经被证明可行的 follow-up action 里，哪条应该成为下一条可复用基线”。
...
- 主线 1：把现有 `ui.launchApp + ui.waitForText + ui.inputText` 从 app 内 smoke，推进成更稳定的跨 app 真机闭环；`ui.swipe` 的第一条真机证据已经有了，下一步只在确实有价值时再考虑把它收成 repo-side harness。
...
- `ui.swipe` 已经进入当前命令面、远端 write gate、nodes tooling、Connect tab 文案和 `/examples`，本地 Kotlin 编译与定向单测都已通过，而且设置首页滚动这条真机证据也已经补齐；当前只缺“要不要把它做成 repo-side follow-up harness”的判断。
...
6. 如果要推进跨 app follow-up，优先跑 `pnpm android:local-host:ui:cross-app:next`；如果目标流程确实需要滚动，先复用已经验证过的 `ui.swipe` 坐标模式，再判断是否值得把它抽成脚本入口
...
4. 再读 `apps/android/local-host-ui-automation-plan.md` 里的当前 next-session plan，先复跑现有 `ui.launchApp + ui.inputText` 跨 app 路径；如果目标界面需要滚动，先复用已验证过的 `ui.swipe` 坐标模式，再判断是否值得把它脚本化，并继续记录 OPPO 后台冻结。Then read the current next-session plan in `apps/android/local-host-ui-automation-plan.md`, replay the existing `ui.launchApp + ui.inputText` cross-app path first; if the target UI needs scrolling, reuse the validated coordinate-based `ui.swipe` pattern first, then decide whether it is worth scripting, and keep documenting the OPPO background freeze.
