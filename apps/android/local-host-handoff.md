...
2. 先把 `ui.launchApp` 做成可重复的真机路径，并调查当前 OPPO / ColorOS 的后台冻结风险，再继续最缺的跨 app 动作，优先 `input_text`。First turn `ui.launchApp` into a repeatable real-device path and investigate the current OPPO / ColorOS background-freeze risk, then continue with the next missing cross-app action, with `input_text` first.
...
3. 再读 `apps/android/local-host-ui-automation-plan.md` 里的当前 next-session plan，优先把现有闭环做成 smoke、补 `ui.launchApp` 的后续检查并记录 OPPO 后台冻结，再继续 `input_text`。Then read the current next-session plan in `apps/android/local-host-ui-automation-plan.md`, first turn the existing loop into a smoke path, add the `ui.launchApp` follow-up checks and record the OPPO background freeze, and then continue with `input_text`.
