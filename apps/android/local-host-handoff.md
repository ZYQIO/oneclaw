...
As of March 29, 2026, the MVP happy path is working, the streaming gate has direct on-device proof, the first repeatable UI smoke passes, the cross-app probe plus settings-search follow-up already have real-device evidence, the repo-side `ui.swipe` follow-up now also has real-device evidence with before/after `visibleText` samples in `summary.json`, and the first multi-window sweep still shows no failure within 30 seconds. The active engineering focus has therefore moved from "is short-window cross-app reachability real?" to "which of the now-proven follow-up paths should stay as stable baselines, and where do we actually need a higher-level preset or state-reset helper?" / 截至 2026 年 3 月 29 日，MVP 的成功路径已经跑通，streaming 门槛已有真机直证，第一条可重复 UI smoke 已通过，cross-app probe 连同 settings-search follow-up 也已有真机证据，repo-side `ui.swipe` follow-up 现在也已经拿到带前后 `visibleText` 样本的 `summary.json` 真机证据，而第一条多窗口 sweep 仍然证明 30 秒内没有掉线；当前工程重心因此已经从“短窗口跨 app 可达性是不是真的”转到“哪些已经被证明可行的 follow-up 路径应该继续保持成稳定基线，以及哪里才真的需要更高层的 preset 或状态重置 helper”。
...
- `ui.swipe` 已经进入当前命令面、远端 write gate、nodes tooling、Connect tab 文案和 `/examples`，本地 Kotlin 编译与定向单测都已通过，而且现在不仅有设置首页滚动真机证据，也已经有 repo-side follow-up harness + `summary.json` 前后 `visibleText` 样本；当前只缺“要不要继续给它加默认 preset 或状态重置 helper”的判断。
- 如果是复跑 Settings 首页滚动证据，记得先 `adb shell am force-stop com.android.settings` 把目标 app 拉回主页；如果刚跑过 `settings-search-input`，Settings 可能仍停在搜索结果页，此时 swipe 不变更 `visibleText` 应先当作 target-app state drift，而不是 harness 回归。
...
6. 如果要推进跨 app follow-up，优先跑 `pnpm android:local-host:ui:cross-app:next`；如果目标流程确实需要滚动，直接给 `pnpm android:local-host:ui:cross-app` 叠 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_SWIPE_*` 环境变量；如果是重放 Settings 首页滚动证据，先 `adb shell am force-stop com.android.settings`
