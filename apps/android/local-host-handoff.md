...
- app 内中英文切换现在已经扩到更完整的主流程：`Settings -> Language` 会持久化 English / 简体中文 偏好，并立即切换 tab bar、Connect 页、Settings 页、onboarding、Chat / Voice 主界面、Voice 页的 mic/runtime 细状态，以及新暴露出来的 reply / TTS 细状态，还有一批已知 runtime / auth 状态文案；其中 `Codex auth` 默认 OAuth 错误、gateway auth/pairing 常见错误、浏览器里的授权成功/失败页与回调错误页、onboarding 手动配置里的 `token/password/SMS` 次级文案、Chat 页常见失败提示和连接边缘态也已经开始走同一条翻译路径。剩余缺口主要收敛到更深的次级文案、少量连接边缘态和零散错误文本。The in-app bilingual toggle now covers a broader primary flow: `Settings -> Language` persists the English / Simplified Chinese preference and immediately switches the tab bar, Connect tab, Settings tab, onboarding, the main Chat / Voice surfaces, the fine-grained mic/runtime status on the Voice screen, the newly surfaced reply / TTS detail states there, and a batch of known runtime/auth status strings; default `Codex auth` OAuth errors, common gateway auth/pairing failures, the browser-side auth success/failure page plus callback-error page, onboarding manual-setup `token/password/SMS` secondary copy, common Chat-surface failures, and connection edge states are also starting to flow through the same translation path. The remaining gaps are now mostly deeper secondary copy, a smaller set of connection edge states, and scattered error text.
...
- `pnpm android:local-host:codex-guard:launchd -- install --env-file <path>` 现在已经是更自然的 macOS 长驻入口；同一条 helper 也提供 `status` 和 `uninstall`。
- 现在如果只是第一次把 launchd guard 配起来，先跑 `pnpm android:local-host:codex-guard:launchd -- write-env` 会更顺手；它会把默认 env 模板写到 state dir，下一个动作就是填入真实 token 并直接 `install`。
- 如果只想先看当前卡在什么阶段，`pnpm android:local-host:codex-guard:launchd -- status --json` 现在已经会直接回 `recommendedAction`，不需要自己再从 `envFileExists/tokenConfigured/installed/loaded` 拼判断。
- 现在更推荐的首次入口已经变成 `pnpm android:local-host:codex-guard:launchd -- setup`：没 env 就先写模板，给了 token 就直接种进去，准备好后还会继续 install / repair；`write-env` 仍然保留给更细粒度的手工控制。
- 现在更推荐的首次入口已经变成 `pnpm android:local-host:codex-guard:setup`：没 env 就先写模板，给了 token 就直接种进去，准备好后还会继续 install / repair；`pnpm android:local-host:codex-guard:write-env` 仍然保留给更细粒度的手工控制。
