...
As of March 29, 2026, the MVP happy path is working, the streaming gate has direct on-device proof, the first repeatable UI smoke passes, the cross-app probe plus settings-search follow-up already have real-device evidence, and the first multi-window sweep still shows no failure within 30 seconds. The active engineering focus has therefore moved from "is short-window cross-app reachability real?" to "which follow-up action should become the next repeatable baseline, starting with the existing settings-search path and the newly landed `ui.swipe`?" / 截至 2026 年 3 月 29 日，MVP 的成功路径已经跑通，streaming 门槛已有真机直证，第一条可重复 UI smoke 已通过，cross-app probe 连同 settings-search follow-up 也已有真机证据，而第一条多窗口 sweep 仍然证明 30 秒内没有掉线；当前工程重心因此已经从“短窗口跨 app 可达性是不是真的”转到“哪条 follow-up action 应该成为下一条可重复基线，先从现有 settings-search 路径和新落地的 `ui.swipe` 开始”。
...
- 主线 1：把现有 `ui.launchApp + ui.waitForText + ui.inputText` 从 app 内 smoke，推进成更稳定的跨 app 真机闭环；如果目标界面需要滚动，再开始给 `ui.swipe` 补第一条真机证据。
...
- `pnpm android:local-host:ui:cross-app` 现在已经能承接可选 follow-up harness，而且 `settings-search-input` 这条 follow-up 已经有真机正证据。
...
- `ui.swipe` 已经进入当前命令面、远端 write gate、nodes tooling、Connect tab 文案和 `/examples`，本地 Kotlin 编译与定向单测都已通过；当前只缺真机手势证据。
...
6. 如果要推进跨 app follow-up，优先跑 `pnpm android:local-host:ui:cross-app:next`；如果目标流程确实需要滚动，再补一轮 `ui.swipe`
...
4. 再读 `apps/android/local-host-ui-automation-plan.md` 里的当前 next-session plan，先复跑现有 `ui.launchApp + ui.inputText` 跨 app 路径；如果目标界面需要滚动，再开始补 `ui.swipe` 的真机证据，并继续记录 OPPO 后台冻结。Then read the current next-session plan in `apps/android/local-host-ui-automation-plan.md`, replay the existing `ui.launchApp + ui.inputText` cross-app path first; if the target UI needs scrolling, start adding on-device proof for `ui.swipe`, and keep documenting the OPPO background freeze.
