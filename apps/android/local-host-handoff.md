...
As of March 29, 2026, the MVP happy path is working, the streaming gate has direct on-device proof, the first repeatable UI smoke passes, the cross-app probe has real-device evidence, and the first multi-window sweep still shows no failure within 30 seconds. The active engineering focus has therefore moved from "is short-window cross-app reachability real?" to "what is the next repeatable follow-up action while another app stays on top?" / 截至 2026 年 3 月 29 日，MVP 的成功路径已经跑通，streaming 门槛已有真机直证，第一条可重复 UI smoke 已通过，cross-app probe 已拿到真机证据，而第一条多窗口 sweep 也仍然证明 30 秒内没有掉线；当前工程重心因此已经从“短窗口跨 app 可达性是不是真的”转到“当另一个 app 仍在前台时，下一条可重复 follow-up action 是什么”。
...
- cross-app follow-up harness 现在还多了第一条内置 preset：`OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=settings-search-input`。它会在 `apps/android/scripts/local-host-ui-cross-app-probe.sh --describe` 里直接展开成 `wait(Settings) -> tap(Search) -> input(openclaw)` 的 repo-side 复跑提示，并保留显式 env override；所以下一轮补 `ui.launchApp + ui.inputText` 真机证据时，优先从这个 preset 入口起步。The cross-app follow-up harness now also has its first built-in preset as `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=settings-search-input`. In `apps/android/scripts/local-host-ui-cross-app-probe.sh --describe` it expands directly into a repo-side rerun hint for `wait(Settings) -> tap(Search) -> input(openclaw)` while preserving explicit env overrides, so the next real-device `ui.launchApp + ui.inputText` proof should start from this preset entrypoint.
...
- app 内中英文切换已经覆盖 post-onboarding 主页面、onboarding、Chat / Voice 主界面、Voice 页 mic/runtime 细状态、Voice reply / TTS 细状态，以及一批已知 runtime / auth 状态文本；`Codex auth` 的默认 OAuth 错误文案、登录提示页/登录状态文案、gateway auth/pairing 常见连接态、Chat 页常见失败提示、浏览器里的授权成功/失败页与回调错误页、Connect / onboarding 里那组 `Setup Code / Host / Token / Password` 高频表单文案、OAuth 上游动态失败，以及 onboarding 手动配置里的 `token/password/SMS` 次级文案也开始纳入同一路径，并通过本地编译 + `AppStringsTest` + `ConnectTabScreenStringsTest` + `OpenAICodexAuthManagerTest` + `OpenAICodexOAuthApiTest` + `OpenAICodexResponsesClientTest` + `SecurePrefsTest` 复核。
- OpenClaw app 一侧的 dedicated readiness 已基本到位，当前主要缺口在 DPC provisioning。
- dedicated 相关仓库命令现在已经形成链路：`readiness -> testdpc-install -> testdpc-qr -> post-provision -> testdpc-kiosk`；同时 `readiness` 本身也已经会直接给出当前推荐的下一条 repo 命令。
