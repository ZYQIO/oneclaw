...
2. 继续按两条线并行推进：一条是把 `ui.launchApp + ui.inputText` 或其他 follow-up action 推进到 launched app 仍在前台的窗口里；另一条是按 `pnpm android:local-host:dedicated:readiness` 的结论，把专机部署优先收敛到 `Device Owner` 前置清理与 DPC 安装，而不是直接跳去 root/systemize。Continue in two parallel lanes: first, move `ui.launchApp + ui.inputText` or another follow-up action into the window where the launched app is still on top; second, follow `pnpm android:local-host:dedicated:readiness` and converge the dedicated-device track on pre-Device-Owner cleanup plus DPC install rather than jumping straight to root/systemize.
