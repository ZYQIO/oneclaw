...
Record the exact real-device verification steps for this branch, the already-completed `0.6.0` and `0.7.0` results, plus the current `0.11.0` process-runtime-observation bootstrap replay that still needs fresh device proof. / 记录这条分支的真机验证步骤、已经完成的 `0.6.0` 与 `0.7.0` 结果，以及当前仍待补真机证据的 `0.11.0` process-runtime-observation bootstrap 复跑。
...
- On the newest `0.11.0` build, the branch should now ideally converge to `runtimeDescribeAfter.mainlineStatus=process_runtime_observation_bootstrapped` with `runtimeDescribeAfter.recommendedNextSlice=process_runtime_recovery`.
...
- The `pod-runtime-execute.json` artifact that captures the new `runtime-smoke-process-model.json`, `runtime-smoke-activation-contract.json`, `runtime-smoke-supervision-contract.json`, and `runtime-smoke-observation-contract.json` bootstrap state.
...
- Do not treat the repo-side `0.11.0` process-runtime observation bootstrap proof as if the device has already replayed it; the latest real-device proof still stops at `0.7.0` `plugin_lane_replayed`.
