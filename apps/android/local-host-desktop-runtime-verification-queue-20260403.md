...
Record the exact real-device verification steps for this branch, the already-completed `0.6.0` and `0.7.0` results, plus the current `0.17.0` process-runtime-active-session-device-proof baseline that now has fresh device proof and still needs live active-session proof. / 记录这条分支的真机验证步骤、已经完成的 `0.6.0` 与 `0.7.0` 结果，以及当前已经拿到新鲜 device proof、但仍待补 live active-session proof 的 `0.17.0` process-runtime-active-session-device-proof 基线。
...
- Later on April 6, 2026, the same `PFEM10` device replayed that `0.17.0` build again and the combined doctor result converged to `classification=process_runtime_active_session_device_proof_bootstrapped` with `manifestVersion=0.17.0`, `verifiedFileCount=26`, and `runtimeDescribeAfter.recommendedNextSlice=process_runtime_active_session_live_proof`. The updated browser-lane smoke also reran `runtime-smoke` after browser replay was ready and captured `runtimeExecuteAfterBrowser.longLivedProcessReady=true`, `processStatus=standby`, `supervisionStatus=active`, `activeSessionStatus=ready`, `activeSessionValidationStatus=pending_device_proof`, and `activeSessionDeviceProofStatus=pending_live_proof`. The remaining gap in this queue is therefore live active-session proof capture itself. / 2026 年 4 月 6 日稍后，同一台 `PFEM10` 真机又把这版 `0.17.0` build 重新跑通，组合 doctor 结果已经收敛到 `classification=process_runtime_active_session_device_proof_bootstrapped`，并得到 `manifestVersion=0.17.0`、`verifiedFileCount=26` 与 `runtimeDescribeAfter.recommendedNextSlice=process_runtime_active_session_live_proof`。更新后的 browser-lane smoke 还会在 browser replay 就绪后补跑一次 `runtime-smoke`，并留下 `runtimeExecuteAfterBrowser.longLivedProcessReady=true`、`processStatus=standby`、`supervisionStatus=active`、`activeSessionStatus=ready`、`activeSessionValidationStatus=pending_device_proof` 与 `activeSessionDeviceProofStatus=pending_live_proof`。因此这条队列当前剩下的缺口已经不再是“再补一轮 device bootstrap”，而是 live active-session proof 本身。
...
- Step 4 should now also leave `runtimeExecuteAfterBrowser.longLivedProcessReady=true`, `runtimeExecuteAfterBrowser.processStatus=standby`, and `runtimeExecuteAfterBrowser.activeSessionStatus=ready` while the validation/device-proof layer still remains `pending_device_proof` / `pending_live_proof`.
- On the newest `0.17.0` build, the branch should now converge to `runtimeDescribeAfter.mainlineStatus=process_runtime_active_session_device_proof_bootstrapped` with `runtimeDescribeAfter.recommendedNextSlice=process_runtime_active_session_live_proof`.
...
- Do not treat the new `process_runtime_active_session_device_proof_bootstrapped` device proof as if the live active session is already observed; even the refreshed `PFEM10` replay still honestly reports `activeSessionObserved=false` and `activeSessionDeviceProofStatus=pending_live_proof`.
