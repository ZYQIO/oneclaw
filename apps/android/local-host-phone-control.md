...
- The same cross-app probe now also has an optional repo-side follow-up harness: with env vars it can run `ui.waitForText`, `ui.tap`, `ui.inputText`, and a final `ui.state` inside the launched app before the reachability polling begins. This is now ready as the next validation entrypoint, but it still needs fresh real-device proof. / 同一条 cross-app probe 现在还多了一层可选的 repo-side follow-up harness：通过环境变量，它可以在 reachability polling 前先在目标 app 里执行 `ui.waitForText`、`ui.tap`、`ui.inputText` 和最终 `ui.state`。这已经是下一条验证入口，但还需要新的真机正证据。
...
- Fresh real-device proof for the new cross-app follow-up harness that combines `ui.launchApp` and `ui.inputText`; the repo entrypoint now exists, but the positive device evidence still needs to be captured. / 现在缺的是新的真机正证据，用来证明这条新的 cross-app follow-up harness 真的把 `ui.launchApp` 和 `ui.inputText` 串了起来；仓库入口已经有了，但还没补齐设备证据。
