...
- The same cross-app probe now also has an optional repo-side follow-up harness: with env vars it can run `ui.waitForText`, `ui.tap`, `ui.inputText`, and a final `ui.state` inside the launched app before the reachability polling begins. It now also waits for `ui.state` to confirm that the target package truly became the active window before those follow-up actions start, and the default `settings-search-input` path already has real-device proof. / 同一条 cross-app probe 现在还多了一层可选的 repo-side follow-up harness：通过环境变量，它可以在 reachability polling 前先在目标 app 里执行 `ui.waitForText`、`ui.tap`、`ui.inputText` 和最终 `ui.state`。现在它还会先等待 `ui.state` 确认目标 package 真的成为 active window，再开始这些 follow-up 动作，而默认的 `settings-search-input` 路径也已经有真机正证据。
...
- The next gap is no longer “does the cross-app follow-up harness work at all,” but “which proved follow-up actions should be turned into reusable repo-side steps next,” especially whether `ui.swipe` should stay an explicit coordinate action or graduate into a harnessed path. / 现在缺的已经不再是“cross-app follow-up harness 到底能不能工作”，而是“哪些已经被证明可行的 follow-up action 值得继续收成 repo-side 步骤”，尤其是 `ui.swipe` 应继续保持显式坐标动作，还是升级成 harness 路径。
