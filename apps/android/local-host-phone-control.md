...
- Later that night, the same repo-side follow-up lane also gained a second non-Settings preset as `calculator-home-open-conversion`: with `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`, `pnpm android:local-host:ui:cross-app:next` now keeps `classification=foregrounded_host_reachable`, `tap_ok=true`, and `state_ok=true` while the final `followUp.stateVisibleText` sample shifts into Calculator conversion entries such as `汇率`, `房贷`, `长度`, and `面积`. / 同一天深夜，这条 repo-side follow-up 线路又新增了第二条非 Settings preset：`calculator-home-open-conversion`。在叠加 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` 后，`pnpm android:local-host:ui:cross-app:next` 现在可以继续保持 `classification=foregrounded_host_reachable`、`tap_ok=true` 和 `state_ok=true`，同时最终的 `followUp.stateVisibleText` 会切到 `汇率`、`房贷`、`长度`、`面积` 这一组 Calculator 换算页面文本。
