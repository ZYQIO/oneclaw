...
- Later on March 29, 2026, `ui.launchApp` was further hardened for vendor system apps: after adding `MAIN/LAUNCHER` package-visibility `queries` plus an explicit launcher-resolve fallback, the current OPPO / ColorOS phone stopped misclassifying `com.coloros.calculator` and `com.coloros.filemanager` as `APP_NOT_INSTALLED`; both now return `launched=true`, and Calculator also went on to pass a 4-second cross-app probe as `foregrounded_host_reachable`. / 2026 年 3 月 29 日稍后，`ui.launchApp` 又针对 vendor 系统 app 做了一次加固：在补上 `MAIN/LAUNCHER` package-visibility `queries` 和显式 launcher resolve fallback 之后，当前 OPPO / ColorOS 手机不再把 `com.coloros.calculator` 与 `com.coloros.filemanager` 误判成 `APP_NOT_INSTALLED`；这两个包现在都能返回 `launched=true`，其中 Calculator 还进一步通过了 4 秒 cross-app probe，并得到 `foregrounded_host_reachable`。
...
- On March 29, 2026, that in-app smoke also needed one selector hardening on the current OPPO / ColorOS phone: tapping the bottom-nav `text=\"Chat\"` label stopped switching tabs reliably, while `contentDescription=\"Chat\"` remained stable, so the baseline now uses the content-description selector. / 2026 年 3 月 29 日，这条 in-app smoke 在当前 OPPO / ColorOS 手机上还补了一次 selector 加固：底部导航里点击 `text=\"Chat\"` 已经不再稳定切页，而 `contentDescription=\"Chat\"` 仍然稳定，所以当前基线改成了 content-description selector。
...
- The same cross-app probe now also has an optional repo-side follow-up harness: with env vars it can run `ui.waitForText`, `ui.swipe`, `ui.tap`, `ui.inputText`, and a final `ui.state` inside the launched app before the reachability polling begins. It now also waits for `ui.state` to confirm that the target package truly became the active window before those follow-up actions start, the default `settings-search-input` path already has real-device proof, the repo-side swipe path now records before/after `visibleText` samples plus `swipeVisibleTextChanged` in `summary.json`, and the new `settings-home-swipe-up` preset resolves ratio-based swipe points against the current window bounds instead of requiring one device-specific coordinate set. / 同一条 cross-app probe 现在还多了一层可选的 repo-side follow-up harness：通过环境变量，它可以在 reachability polling 前先在目标 app 里执行 `ui.waitForText`、`ui.swipe`、`ui.tap`、`ui.inputText` 和最终 `ui.state`。现在它还会先等待 `ui.state` 确认目标 package 真的成为 active window，再开始这些 follow-up 动作；默认的 `settings-search-input` 路径已经有真机正证据，repo-side swipe 路径现在也会把前后 `visibleText` 样本和 `swipeVisibleTextChanged` 写进 `summary.json`，而新的 `settings-home-swipe-up` preset 还会先根据当前窗口 bounds 解析 ratio 形式的滑动起止点，不再要求一套只适用于单一设备的绝对坐标。
- On March 29, 2026, that `settings-home-swipe-up` preset also passed on-device through `pnpm android:local-host:ui:cross-app:next`, again classifying as `foregrounded_host_reachable` while recording `swipeCoordinateMode=ratio` and a positive `swipeVisibleTextChanged=true`. / 2026 年 3 月 29 日，这条 `settings-home-swipe-up` preset 也已经通过 `pnpm android:local-host:ui:cross-app:next` 在真机跑通，再次得到 `foregrounded_host_reachable`，同时落下 `swipeCoordinateMode=ratio` 和正向的 `swipeVisibleTextChanged=true`。
- Later the same day, the probe also validated an opt-in target reset before launch: after first leaving Settings in its search surface through `settings-search-input`, rerunning `settings-home-swipe-up` with `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` still classified as `foregrounded_host_reachable`, recorded `targetResetApplied=true`, and preserved the positive `swipeVisibleTextChanged=true` result. / 同一天稍后，这条 probe 还验证了一个 launch 前的 opt-in 目标 app reset：先用 `settings-search-input` 把 Settings 留在搜索态后，再用 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` 复跑 `settings-home-swipe-up`，结果仍然得到 `foregrounded_host_reachable`、记录下 `targetResetApplied=true`，并继续保持正向的 `swipeVisibleTextChanged=true`。
- Later that night, the same repo-side follow-up lane also gained a second non-Settings preset as `calculator-home-open-conversion`: with `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`, `pnpm android:local-host:ui:cross-app:next` now keeps `classification=foregrounded_host_reachable`, `tap_ok=true`, `final_wait_ok=true`, and `state_ok=true`, with `followUp.finalWaitMatchedText=汇率` and the final `followUp.stateVisibleText` sample both confirming the switch into Calculator conversion entries such as `汇率`, `房贷`, `长度`, and `面积`. / 同一天深夜，这条 repo-side follow-up 线路又新增了第二条非 Settings preset：`calculator-home-open-conversion`。在叠加 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` 后，`pnpm android:local-host:ui:cross-app:next` 现在可以继续保持 `classification=foregrounded_host_reachable`、`tap_ok=true`、`final_wait_ok=true` 和 `state_ok=true`；同时 `followUp.finalWaitMatchedText=汇率` 与最终的 `followUp.stateVisibleText` 会一起确认已经切到 `汇率`、`房贷`、`长度`、`面积` 这一组 Calculator 换算页面文本。
...
- The next gap is no longer “does the cross-app follow-up harness work at all,” nor “can `ui.swipe` be harnessed,” nor even “can one reusable swipe preset or one opt-in reset helper work across reruns.” It is now “where are additional presets or reset helpers actually worth it without overfitting one OEM surface.” / 现在缺的已经不再是“cross-app follow-up harness 到底能不能工作”，也不再是“`ui.swipe` 能不能接进 harness”，甚至连“能不能做出一条可复用的 swipe preset 或一条 opt-in reset helper”都不再是问题；真正剩下的是“哪里还值得补更多 preset 或 reset helper，同时又不至于过拟合某一台 OEM 设备的界面”。
