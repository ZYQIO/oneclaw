...
- The next gap is no longer “does the cross-app follow-up harness work at all,” nor “can `ui.swipe` be harnessed,” nor even “can one reusable swipe preset work across screens.” It is now “where are additional presets or state-reset helpers actually worth it without overfitting one OEM surface.” / 现在缺的已经不再是“cross-app follow-up harness 到底能不能工作”，也不再是“`ui.swipe` 能不能接进 harness”，甚至连“能不能做出一条可复用的 swipe preset”都不再是问题；真正剩下的是“哪里还值得补更多 preset 或状态重置 helper，同时又不至于过拟合某一台 OEM 设备的界面”。
