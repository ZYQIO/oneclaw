...
private fun localizeRuntimeErrorCode(
  ...
  return when (trimmed) {
    ...
    "NOTIFICATION_FAILED" -> language.pick(trimmed, "通知失败")
...
private fun localizeRuntimeErrorDetail(
  ...
  return when (trimmed) {
    ...
    "notification has no reply action" -> language.pick(trimmed, "通知没有可回复的操作")
    ...
    "notification post failed" -> language.pick(trimmed, "发送失败")
    "notifications" -> language.pick(trimmed, "通知权限")
    ...
    "replyText required for reply action" -> language.pick(trimmed, "回复操作需要提供 replyText")
    "request failed" -> language.pick(trimmed, "请求失败")
