...
- Landed fourth helper: `pod.manifest.describe` for the packaged pod manifest/layout metadata, with no network and no browser. / 已落地的第四个 helper：返回打包 pod manifest / layout 元数据的 `pod.manifest.describe`，不访问网络、不启动浏览器。
...
If we need another command later, it should still be offline and deterministic. / 如果后面还要补新的命令，也应保持离线和确定性。
...
- `apps/android/app/src/main/java/ai/openclaw/app/node/PodHandler.kt` now holds the read-only helper entrypoints `pod.health`, `pod.manifest.describe`, `pod.workspace.scan`, and `pod.workspace.read`, which expose pod readiness, packaged manifest/layout metadata, packaged workspace inventory, and one packaged workspace document through `invoke`. / `apps/android/app/src/main/java/ai/openclaw/app/node/PodHandler.kt` 现在承载只读 helper 入口 `pod.health`、`pod.manifest.describe`、`pod.workspace.scan` 与 `pod.workspace.read`，并通过 `invoke` 暴露 pod readiness、打包 manifest / layout metadata、打包 workspace inventory 和一份打包 workspace 文档。
...
### Phase 2. Deterministic Helper Workflows / 确定性 Helper 工作流

- Land `pod.health` first, then keep every additional offline helper tied to a concrete reduction in duplicated logic. / 先落地 `pod.health`，然后让后续每一条离线 helper 都对应一次明确的重复逻辑收敛。
...
- Status after April 2, 2026: `pod.health`, `pod.manifest.describe`, `pod.workspace.scan`, and `pod.workspace.read` are all landed through read-only `invoke` plus the `nodes` action surface, while `/status` continues to expose the same underlying readiness snapshot; `pod.manifest.describe` now returns packaged manifest/layout metadata from the installed pod, `pod.workspace.scan` returns the extracted workspace stage manifest, `content-index`, filtered file inventory, and preview text from the packaged `0.2.0` payload, and `pod.workspace.read` returns one packaged workspace document by relative path with text and metadata. / 截至 2026 年 4 月 2 日：`pod.health`、`pod.manifest.describe`、`pod.workspace.scan` 与 `pod.workspace.read` 都已通过只读 `invoke` 与 `nodes` action 落地，而 `/status` 继续暴露同一份底层 readiness 快照；`pod.manifest.describe` 现在会从已安装 pod 返回打包 manifest / layout metadata，`pod.workspace.scan` 会从打包后的 `0.2.0` payload 中返回解包 workspace 的 stage manifest、`content-index`、过滤后的文件清单和 preview text，而 `pod.workspace.read` 会按相对路径返回一份打包 workspace 文档及其文本与元数据。
...
- That same on-device rerun now also verifies `hasPodManifestDescribe=true`, `manifestSource=installed`, `layoutSource=installed`, `stageCount=3`, `directoryCount=3`, `fileCount=7`, and `workspaceStageFileCount=5`, so the packaged manifest/layout metadata is no longer inferred indirectly from repo artifacts alone. / 同一条真机复跑现在还额外验证了 `hasPodManifestDescribe=true`、`manifestSource=installed`、`layoutSource=installed`、`stageCount=3`、`directoryCount=3`、`fileCount=7` 与 `workspaceStageFileCount=5`，因此打包 manifest / layout metadata 已不再只靠 repo 侧工件间接推断。
...
- The packaging/extraction/verifier path plus the helper quartet `pod.health`, `pod.manifest.describe`, `pod.workspace.scan`, and `pod.workspace.read` are now landed and already have a boring real-device proof; before adding any fifth offline helper, keep the current quartet replayable with `pnpm android:local-host:embedded-runtime-pod:smoke`, keep stale-build diagnostics obvious, and only then expand the helper surface. / 打包 / 解包 / 校验路径加 helper quartet `pod.health`、`pod.manifest.describe`、`pod.workspace.scan` 与 `pod.workspace.read` 现在都已经落地，并且已有一份“无聊地可靠”的真机证据；在考虑任何第五条离线 helper 之前，先继续用 `pnpm android:local-host:embedded-runtime-pod:smoke` 把当前 quartet 维持成可复跑基线，把 stale build 诊断保持清晰，然后再扩 helper 面。
- The next bounded expansion should now optimize for stability or a very clear reduction in duplicated logic rather than basic file consumption or basic pod metadata inspection, because `pod.workspace.read` plus `pod.manifest.describe` already prove the phone can read packaged documents and packaged manifest/layout metadata directly. / 既然 `pod.workspace.read` 与 `pod.manifest.describe` 已经证明手机可以直接读取打包文档和打包 manifest / layout metadata，下一条有边界的扩展现在应优先考虑稳定性或非常明确的重复逻辑收敛，而不是继续补最基础的文件消费或 metadata 查看能力。
