...
- After you finish the external browser auth flow on the device, rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0` to confirm that the stored credential path has converged into a configured desktop/plugin/process-runtime-supervision bootstrap state such as `process_runtime_supervision_bootstrapped`.
...
- `classification=process_runtime_supervision_bootstrapped`: the branch now also leaves a structured supervision-contract artifact on disk, and the next step is to deepen that bootstrap into stronger process observation and recovery semantics
