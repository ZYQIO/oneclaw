...
- The current embedded pod payload is versioned as `0.17.0`, which keeps the packaged `desktop/` environment stage, the first allowlisted packaged plugin descriptor/task, and now also deepens `runtime-smoke` with structured process-model, activation-contract, supervision-contract, observation-contract, recovery-contract, detached-launch-contract, supervisor-loop-contract, active-session-contract, active-session-validation, and active-session-device-proof bootstrap artifacts while still extracting into a fresh app-private version directory instead of silently reusing the previous payload.
- On April 6, 2026, repo-side validation on this branch verified the repeated-replay live-proof slice: `pnpm test -- apps/android/runtime-pod/prepare.test.ts apps/android/runtime-pod/sync-assets.test.ts` plus the targeted Android Gradle suite pass, and `runtime-smoke` now writes `runtime-smoke-process-model.json`, `runtime-smoke-activation-contract.json`, `runtime-smoke-supervision-contract.json`, `runtime-smoke-observation-contract.json`, `runtime-smoke-recovery-contract.json`, `runtime-smoke-detached-launch-contract.json`, `runtime-smoke-supervisor-loop-contract.json`, `runtime-smoke-active-session-contract.json`, `runtime-smoke-active-session-validation.json`, and `runtime-smoke-active-session-device-proof.json`. Later that same day, a fresh `PFEM10` replay on payload `0.17.0` also converged to `classification=process_runtime_active_session_live_proof_captured`; the updated browser-lane smoke now reruns `runtime-smoke` after browser replay is ready and records `runtimeExecuteAfterBrowser.longLivedProcessReady=true`, `processStatus=standby`, `supervisionStatus=active`, `activeSessionStatus=ready`, `activeSessionObserved=true`, `activeSessionValidationStatus=validated`, and `activeSessionDeviceProofStatus=verified`. The doctor wrapper now also auto-reruns one confirm-only browser-lane pass with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0`, and the same `PFEM10` replay leaves `confirmBrowserLaneSmoke.required=true`, `executed=true`, `ok=true`, `mainlineStatus=process_runtime_active_session_live_proof_captured`, and `liveProofReplayed=true`.
- This is now a real packaging-plus-extraction-plus-runtime-carrier path with a cohesive desktop bundle in the APK plus app-private desktop-home materialization, profile replay, environment supervision artifacts, one narrow allowlisted plugin replay, structured process-model plus activation-contract plus supervision-contract plus observation-contract plus recovery-contract plus detached-launch-contract plus supervisor-loop-contract plus active-session-contract plus active-session-validation plus active-session-device-proof bootstrap artifacts, and one bounded captured live active-session proof. It is still not a full embedded desktop runtime yet: generic browser tooling, unrestricted shell parity, and broader detached-process hardening remain incomplete.
...
Use this when you want a bounded, no-chat smoke for the packaged pod helpers themselves.
...
Each run writes `status.json`, `capabilities.json`, `pod-health.json`, `pod-manifest-describe.json`, `pod-browser-describe.json`, `pod-runtime-describe.json`, `pod-runtime-execute.json`, `pod-tool-execute.json`, `pod-workspace-scan.json`, `pod-workspace-read.json`, and `summary.json` into the artifact directory so the phone-side pod state is easy to inspect after the fact.
...
Artifact output includes `pod-desktop-materialize.json`, `pod-browser-describe-before.json`, `pod-browser-auth-start.json`, `pod-browser-describe-after.json`, `pod-runtime-execute-after-browser.json`, `pod-runtime-describe-after.json`, `pod-plugin-execute.json`, and `summary.json`.

- Use the default `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=1` pass to prove that the packaged browser lane leaves replayable state/log evidence on disk while the packaged desktop home and allowlisted plugin lane are also replayed from the current APK bundle, then let the helper rerun `runtime-smoke` once more so the runtime contracts see the browser dependency as present.
- After you finish the external browser auth flow on the device, rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0` to confirm that the stored credential path has converged into a configured desktop/plugin/live-proof state such as `process_runtime_active_session_live_proof_captured`.
...
- when the first browser-lane pass reaches live proof with browser start enabled, auto-runs one confirm-only browser-lane rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0`
- writes one combined `summary.json` that includes token bootstrap info, the pod smoke summary, the browser-lane smoke summary, the confirm-only browser-lane summary, and a single top-level classification
...
- `classification=process_model_bootstrapped`: the branch now also leaves a structured process-model artifact on disk, and the next step is to deepen that bootstrap into a real supervised runtime activation path
- `classification=process_runtime_activation_bootstrapped`: the branch now also leaves a structured activation-contract artifact on disk, and the next step is to deepen that bootstrap into a supervision contract
- `classification=process_runtime_supervision_bootstrapped`: the branch now also leaves a structured supervision-contract artifact on disk, and the next step is to deepen that bootstrap into an observation contract
- `classification=process_runtime_observation_bootstrapped`: the branch now also leaves a structured observation-contract artifact on disk, and the next step is to deepen that bootstrap into an explicit recovery contract
- `classification=process_runtime_detached_launch_bootstrapped`: the branch now also leaves a structured detached-launch-contract artifact on disk, and the next step is to deepen that bootstrap into a supervisor-loop contract
- `classification=process_runtime_supervisor_loop_bootstrapped`: the branch now also leaves a structured supervisor-loop-contract artifact on disk, and the next step is to deepen that bootstrap into an explicit active-session contract
- `classification=process_runtime_active_session_bootstrapped`: the branch now also leaves a structured active-session-contract artifact on disk, and the next step is to deepen that bootstrap into an explicit active-session-validation contract
- `classification=process_runtime_active_session_validation_bootstrapped`: the branch now also leaves a structured active-session-validation artifact on disk, and the next step is to deepen that bootstrap into an explicit active-session-device-proof contract
