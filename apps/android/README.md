...
- After you finish the external browser auth flow on the device, rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0` to confirm that the stored credential path has converged into a configured desktop/plugin/process-runtime-recovery bootstrap state such as `process_runtime_recovery_bootstrapped`.
...
- `classification=process_runtime_recovery_bootstrapped`: the branch now also leaves a structured recovery-contract artifact on disk, and the next step is to deepen that bootstrap into a detached process launch loop
