...
A third repo preset now exists as `calculator-home-open-conversion`: it targets `com.coloros.calculator`, taps the stable `com.coloros.calculator:id/item_open_conversion` resource ID, and turns the current OPPO / ColorOS Calculator homepage into a second replayable non-Settings follow-up surface. When you want a clean Calculator homepage first, pair it with `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`.
