...
- The first UI-automation phone-control slice now exists separately from the MVP close-out surface: `ui.state` and `ui.waitForText` stay read-only by default, while `ui.launchApp`, `ui.tap`, `ui.back`, and `ui.home` sit behind the remote write tier.
