...
- The current embedded pod payload is versioned as `0.9.0`, which keeps the packaged `desktop/` environment stage, the first allowlisted packaged plugin descriptor/task, and now also deepens `runtime-smoke` with both a structured process-model artifact and a first activation-contract bootstrap artifact while still extracting into a fresh app-private version directory instead of silently reusing the previous payload.
- On April 5, 2026, repo-side validation on this branch verified the new slice: `pnpm test -- apps/android/runtime-pod/prepare.test.ts apps/android/runtime-pod/sync-assets.test.ts` plus the targeted Android Gradle suite pass, and `runtime-smoke` now writes both `runtime-smoke-process-model.json` and `runtime-smoke-activation-contract.json`. The latest completed real-device replay remains the April 4, 2026 `PFEM10` proof for `classification=plugin_lane_replayed` on payload `0.7.0`; the next device target is `classification=process_runtime_activation_bootstrapped`.
- This is now a real packaging-plus-extraction-plus-runtime-carrier path with a cohesive desktop bundle in the APK plus app-private desktop-home materialization, profile replay, environment supervision artifacts, one narrow allowlisted plugin replay, and structured process-model plus activation-contract bootstrap artifacts, but it is still not a full embedded desktop runtime yet: generic browser tooling, unrestricted shell parity, and a real long-lived supervised process remain incomplete.
...
- After you finish the external browser auth flow on the device, rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0` to confirm that the stored credential path has converged into a configured desktop/plugin/process-runtime-activation bootstrap state such as `process_runtime_activation_bootstrapped`.
...
- `classification=process_runtime_activation_bootstrapped`: the branch now also leaves a structured activation-contract artifact on disk, and the next step is to deepen that bootstrap into a real long-lived supervised runtime process
