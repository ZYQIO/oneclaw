...
- After you finish the external browser auth flow on the device, rerun with `OPENCLAW_ANDROID_LOCAL_HOST_BROWSER_START=0` to confirm that the stored credential path has converged into a configured desktop/plugin/process-runtime-observation bootstrap state such as `process_runtime_observation_bootstrapped`.
...
- `classification=process_runtime_observation_bootstrapped`: the branch now also leaves a structured observation-contract artifact on disk, and the next step is to deepen that bootstrap into explicit recovery actions
