...
- 继续把 MVP 从“happy path 已通”推进到“验证闭环完整”。Move the MVP from "happy path works" to "validation loop is complete."

### Priority Order / 优先顺序

1. 验证 Codex refresh 成功路径。Validate a successful Codex refresh path.
   Exit criteria / 退出标准:
   - 设备上的 Codex 凭证接近过期或被人工缩短有效期后，聊天仍能成功返回。
   - 刷新后的凭证被重新保存，App 重启后仍能继续使用。
2. 记录权限缺失类失败场景。Capture permission-missing failure paths.
   Exit criteria / 退出标准:
   - 至少覆盖一个相机类命令、一个位置类命令、一个通知或联系人类命令。
   - 错误响应文案清晰，且被写回 `local-host-self-check.md`。
3. 复核远程访问默认值与网络指引。Review remote-access defaults and network guidance.
   Exit criteria / 退出标准:
   - 明确默认只推荐 LAN / trusted tunnel。
   - 明确 token 轮换和高风险命令层的使用建议。
4. 判断 MVP 命令面是否收口。Decide whether the MVP remote-command surface is final enough.
   Exit criteria / 退出标准:
   - 若无需新增命令，明确写入文档。
   - 若仍需新增命令，只为真实场景补最小集合。
...
- 在做任何功能扩展前，优先补 refresh 和权限失败证据。Before expanding features, prioritize refresh and permission-failure evidence.
