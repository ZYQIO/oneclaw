...
- 这轮又把 app 内中英文切换继续往前推了一段：在原有 `AppLanguage` 持久化、`RootScreen` 语言上下文，以及 `Settings -> Language` 入口基础上，`OnboardingFlow` 已覆盖步骤条、权限页、Review 页和权限状态文案；同时 Chat / Voice 主界面、Voice 页 mic/runtime 细状态，以及一批已知 runtime / auth 状态文本也已经接进同一条语言切换路径，`Codex auth` 的默认 OAuth 错误文案、gateway auth/pairing 常见连接态、Chat 页常见失败提示、浏览器里的授权成功/失败页与回调错误页，以及 onboarding 手动配置里的 `token/password/SMS` 次级文案也开始走同一套翻译。The in-app English / Simplified Chinese toggle moved forward again this round: on top of the persisted `AppLanguage`, the `RootScreen` language context, and the `Settings -> Language` entrypoint, `OnboardingFlow` now covers the step rail, permissions page, review page, and permission-status copy; the main Chat / Voice surfaces, the fine-grained mic/runtime status on the Voice screen, and a batch of known runtime/auth status text are now on the same language-switching path too, while the default `Codex auth` OAuth errors, common gateway auth/pairing connection states, common Chat-surface failures, the browser-side auth success/failure page plus callback-error page, and onboarding manual-setup `token/password/SMS` secondary copy have also started using the same translation layer.
