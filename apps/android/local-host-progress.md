...
2. 先把 `ui.launchApp` 做成可重复的真机路径，并调查当前 OPPO / ColorOS 的后台冻结，再继续下一段最缺的跨 app 动作面，优先 `input_text`。First turn `ui.launchApp` into a repeatable real-device path and investigate the current OPPO / ColorOS background freeze, then continue with the next missing cross-app surface, with `input_text` first.
...
3. 在 `ui.launchApp -> follow-up check` 稳定之后，再补下一批跨 app 动作，优先 `input_text`。After `ui.launchApp -> follow-up check` is stable, add the next cross-app action, with `input_text` first.
   Output / 产出:
...
3. 把第一条 bounded control loop 做成可复跑的验证面，并继续补下一批跨 app 动作。Turn the first bounded control loop into a repeatable verification surface and continue with the next batch of cross-app actions.
   ...
   - `ui.launchApp` 拿到可重复的真机证据并记录 OPPO 后台冻结边界，或 `input_text` 至少开出一条明确实现路径。
