...
2. 既然新的 `pnpm android:local-host:ui:cross-app` 已经把“短窗口跨 app 是否真切前台且仍可达”固定为 `foregrounded_host_reachable`，下一步应把观察窗口拉长并继续验证 `ui.launchApp + ui.inputText`，找出当前 OPPO / ColorOS 设备真正开始冻结后台 Host 的时间边界。Now that `pnpm android:local-host:ui:cross-app` has pinned the short-window cross-app truth as `foregrounded_host_reachable`, the next step should stretch the observation window and keep validating `ui.launchApp + ui.inputText` so we can find the real background-freeze boundary on the current OPPO / ColorOS phone.
...
3. 在 `ui.launchApp -> follow-up check -> ui.inputText` 稳定之后，再补下一批跨 app 动作，优先 `swipe` 或更强 selector。After `ui.launchApp -> follow-up check -> ui.inputText` is stable, add the next cross-app action, with `swipe` or stronger selectors next.
   Output / 产出:
