...
2. 再补下一段最缺的跨 app 动作面，优先 `launch_app` 与 `input_text`。Then add the next missing cross-app surfaces, with `launch_app` and `input_text` first.
...
3. 再补下一批跨 app 动作，优先 `launch_app` 与 `input_text`。Then add the next cross-app actions, with `launch_app` and `input_text` first.
   Output / 产出:
...
3. 把第一条 bounded control loop 做成可复跑的验证面，并继续补下一批跨 app 动作。Turn the first bounded control loop into a repeatable verification surface and continue with the next batch of cross-app actions.
   ...
   - `launch_app` 或 `input_text` 至少开出一条明确实现路径。
