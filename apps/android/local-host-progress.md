...
- 这轮又把 app 内中英文切换继续往前推了一段：在原有 `AppLanguage` 持久化、`RootScreen` 语言上下文，以及 `Settings -> Language` 入口基础上，`OnboardingFlow` 已覆盖步骤条、权限页、Review 页和权限状态文案；同时 Chat / Voice 主界面、Voice 页 mic/runtime 细状态，以及一批已知 runtime / auth 状态文本也已经接进同一条语言切换路径，`Codex auth` 的默认 OAuth 错误文案、gateway auth/pairing 常见连接态，以及浏览器里的授权成功/失败页也开始走同一套翻译。The in-app English / Simplified Chinese toggle moved forward again this round: on top of the persisted `AppLanguage`, the `RootScreen` language context, and the `Settings -> Language` entrypoint, `OnboardingFlow` now covers the step rail, permissions page, review page, and permission-status copy; the main Chat / Voice surfaces, the fine-grained mic/runtime status on the Voice screen, and a batch of known runtime/auth status text are now on the same language-switching path too, while the default `Codex auth` OAuth errors, common gateway auth/pairing connection states, and the browser-side auth success/failure page have also started using the same translation layer.
- 这条语言切换路径已经完成本地验证：`env ANDROID_HOME=/Users/zouxiaoyi/Library/Android/sdk ANDROID_SDK_ROOT=/Users/zouxiaoyi/Library/Android/sdk ./gradlew :app:compileDebugKotlin` 与 `env ANDROID_HOME=/Users/zouxiaoyi/Library/Android/sdk ANDROID_SDK_ROOT=/Users/zouxiaoyi/Library/Android/sdk ./gradlew :app:testDebugUnitTest --tests ai.openclaw.app.auth.OpenAICodexAuthManagerTest --tests ai.openclaw.app.ui.AppStringsTest --tests ai.openclaw.app.ui.ConnectTabScreenStringsTest --tests ai.openclaw.app.SecurePrefsTest` 都成功通过；其中 `AppStringsTest`、`ConnectTabScreenStringsTest` 和 `OpenAICodexAuthManagerTest` 分别锁住了连接/Voice runtime 状态翻译、gateway auth/pairing 常见连接态翻译、`Codex auth` 常见 OAuth 错误翻译，以及浏览器授权页的双语 CTA/提示。The language-toggle path has local verification too: `env ANDROID_HOME=/Users/zouxiaoyi/Library/Android/sdk ANDROID_SDK_ROOT=/Users/zouxiaoyi/Library/Android/sdk ./gradlew :app:compileDebugKotlin` and `env ANDROID_HOME=/Users/zouxiaoyi/Library/Android/sdk ANDROID_SDK_ROOT=/Users/zouxiaoyi/Library/Android/sdk ./gradlew :app:testDebugUnitTest --tests ai.openclaw.app.auth.OpenAICodexAuthManagerTest --tests ai.openclaw.app.ui.AppStringsTest --tests ai.openclaw.app.ui.ConnectTabScreenStringsTest --tests ai.openclaw.app.SecurePrefsTest` both passed; `AppStringsTest`, `ConnectTabScreenStringsTest`, and `OpenAICodexAuthManagerTest` now lock connection/Voice runtime translations, common gateway auth/pairing connection-state translations, common `Codex auth` OAuth-error translations, and the bilingual CTA/hint on the browser auth page into regression coverage.
