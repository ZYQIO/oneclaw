...
1. 先在真机上复跑新的 `pnpm android:local-host:ui`，把默认 in-app smoke 证据补齐。Replay the new `pnpm android:local-host:ui` script on-device first so the default in-app smoke has direct device proof.
2. 再把 `ui.launchApp + ui.inputText` 组合成可重复的跨 app 真机路径，并调查当前 OPPO / ColorOS 的后台冻结，确认这条组合链路何时会被 OEM 打断。Then turn `ui.launchApp + ui.inputText` into a repeatable cross-app device path and investigate the current OPPO / ColorOS background freeze so we know exactly when OEM policy interrupts that combined flow.
...
2. 先跑一次新的 `pnpm android:local-host:ui`，确认默认 in-app smoke 路径可以直接复跑。Run the new `pnpm android:local-host:ui` script once and confirm the default in-app smoke path replays cleanly.
   Output / 产出:
