...
1. 先把当前真机 observe / wait / act 证据做成可重复的 smoke 脚本。Turn the current real-device observe / wait / act proof into a repeatable smoke script first.
2. 先把 `ui.launchApp + ui.inputText` 组合成可重复的真机路径，并调查当前 OPPO / ColorOS 的后台冻结，确认这条组合链路何时会被 OEM 打断。First turn `ui.launchApp + ui.inputText` into a repeatable real-device path and investigate the current OPPO / ColorOS background freeze so we know exactly when OEM policy interrupts that combined flow.
...
2. 把当前 `ui.state` / `ui.waitForText` / `ui.tap` / `ui.home` / `ui.back` 组合成可重复的 smoke 路径。Turn the current `ui.state` / `ui.waitForText` / `ui.tap` / `ui.home` / `ui.back` combination into a repeatable smoke path.
   Output / 产出:
