...
- “第二个真实 cross-app target” 这件事现在已经不再是 blocker：`com.coloros.calculator` 已经通过当前 launcher-query fix 恢复成真机可启动，并在 4 秒 probe 上得到 `foregrounded_host_reachable`。因此下一步暂时不需要为了它单独新增第三个 preset，先继续用 package override 复跑更合适。The question of “do we have a second real cross-app target yet” is no longer a blocker now: `com.coloros.calculator` is launchable again on-device after the current launcher-query fix and already produced `foregrounded_host_reachable` in a 4-second probe. So the next step does not need a third preset just for that target yet; using a package override for replay is the better default for now.
