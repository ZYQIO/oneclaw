...
- Add `launch_app` first so the phone can leave OpenClaw without ADB help. / 先补 `launch_app`，让手机离开 OpenClaw 时不再依赖 ADB 帮忙。
- Add `input_text` next so tasks can move beyond navigation into form-like workflows. / 再补 `input_text`，让任务从纯导航迈向表单类流程。
