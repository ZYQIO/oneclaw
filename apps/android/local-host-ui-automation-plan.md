...
- `ui.state`、`ui.waitForText`、`ui.launchApp`、`ui.inputText`、`ui.tap`、`ui.back`、`ui.home` 都已经落地；其中 `ui.launchApp`、`ui.inputText`、`ui.tap`、`ui.back`、`ui.home` 继续挂在远端 write tier 后面，默认关闭。`ui.state`, `ui.waitForText`, `ui.launchApp`, `ui.inputText`, `ui.tap`, `ui.back`, and `ui.home` are all now landed; `ui.launchApp`, `ui.inputText`, `ui.tap`, `ui.back`, and `ui.home` remain behind the remote write tier and stay off by default.
...
- `apps/android/app/src/main/java/ai/openclaw/app/node/InvokeCommandRegistry.kt` and `apps/android/app/src/main/java/ai/openclaw/app/node/InvokeDispatcher.kt` now expose `ui.state`, `ui.waitForText`, `ui.launchApp`, `ui.inputText`, `ui.tap`, `ui.back`, and `ui.home`, but there is still no `swipe` or launchable-app listing path, and the selector model is still lightweight. / `apps/android/app/src/main/java/ai/openclaw/app/node/InvokeCommandRegistry.kt` 和 `apps/android/app/src/main/java/ai/openclaw/app/node/InvokeDispatcher.kt` 现在已经接入 `ui.state`、`ui.waitForText`、`ui.launchApp`、`ui.inputText`、`ui.tap`、`ui.back`、`ui.home`，但仍然没有 `swipe` 或可启动 app 列表这类路径，而且 selector 模型仍偏轻量。
...
The target is to turn the current manual proof into a reusable validation surface, then add the next missing primitives such as app launch and text entry. / 现在的目标是把当前偏手工的证明路径变成可复用验证面，然后补上 app launch、文本输入等下一批关键 primitive。
...
- 当前第一条内置 preset 已经是 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=settings-search-input`：它默认把目标 app 固定到 `com.android.settings`，并种好 `wait(Settings) -> tap(Search) -> input(openclaw)` 这条 repo-side follow-up 链；如果 OEM 文案不同，继续用显式 env 覆盖即可。The first built-in preset is now `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_PRESET=settings-search-input`: it defaults the target app to `com.android.settings` and seeds a repo-side follow-up chain of `wait(Settings) -> tap(Search) -> input(openclaw)`; if an OEM uses different copy, explicit env overrides still win.
...
- One repeatable cross-app proof that combines `ui.launchApp` and `ui.inputText`, or the next richer primitive such as `swipe`. / 一条把 `ui.launchApp` 和 `ui.inputText` 串起来的可重复跨 app 证据，或者下一条更丰富的 primitive，例如 `swipe`。
...
- At least one task grows beyond the current tap/home/back loop via app launch, text entry, or another bounded primitive. / 至少有一个任务能借助 app launch、文本输入或其他 bounded primitive，超出当前 tap/home/back 的闭环。
