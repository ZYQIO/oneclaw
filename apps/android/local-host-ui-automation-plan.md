...
The target is to turn the current manual proof into a reusable validation surface, then keep the existing cross-app follow-up paths healthy while deciding which already-proven primitives, especially `ui.launchApp`, `ui.inputText`, and the coordinate-first `ui.swipe`, deserve to become reusable harness steps. / 现在的目标是把当前偏手工的证明路径变成可复用验证面，然后在保持现有跨 app follow-up 路径健康的同时，判断哪些已经被证明可行的 primitive 值得收成可复用的 harness 步骤，重点是 `ui.launchApp`、`ui.inputText` 和 coordinate-first 的 `ui.swipe`。
...
- One repeatable cross-app proof that combines `ui.launchApp` and `ui.inputText`, plus a concrete decision on whether the now-proven `ui.swipe` should remain an explicit coordinate tool or graduate into a reusable repo-side follow-up step. / 一条把 `ui.launchApp` 和 `ui.inputText` 串起来的可重复跨 app 证据；以及一个明确判断：已经被证明可行的 `ui.swipe` 应继续保持显式坐标工具，还是该升级成可复用的 repo-side follow-up 步骤。
