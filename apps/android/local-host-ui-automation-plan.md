...
- 同一天深夜又把 Calculator 收成了第二条非 Settings 的 repo-side follow-up preset：`calculator-home-open-conversion` 默认目标包是 `com.coloros.calculator`，点击稳定资源 ID `com.coloros.calculator:id/item_open_conversion`，并把最终 `ui.state` 的 `visibleText` 样本一并写进 `summary.json`。这让第二个真实 cross-app target 不再只是“能 launch”，而是已经变成一条可复跑的 tap + state 基线。Later that night, Calculator was also tightened into the second non-Settings repo-side follow-up preset: `calculator-home-open-conversion` defaults the target package to `com.coloros.calculator`, taps the stable resource ID `com.coloros.calculator:id/item_open_conversion`, and now also writes the final `ui.state` `visibleText` sample into `summary.json`. That means the second real cross-app target is no longer just “launchable,” but has become a replayable tap+state baseline too.
