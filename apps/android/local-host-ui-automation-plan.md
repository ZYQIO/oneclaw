...
- 同一天深夜又补了一层 window 合并策略：`ui.state` 以及 selector 型 `ui.tap` / `ui.inputText` 不再只看 `rootInActiveWindow`，而是优先扫描当前 app 同包名的 accessibility windows，再补系统窗口。真机在飞书 `com.ss.android.lark` 上复验后，`ui.launchApp` + `ui.tap(text="更多")` 之后，`ui.state.visibleText` 现在能稳定带出底部那层里的 `编辑`、`视频会议`、`任务`，不再被状态栏 / 系统 overlay 文本完全盖掉；但更深的模块项例如 `考勤` 仍未变成稳定可回放的路径。Later that night we also added a window-merge policy: `ui.state` plus selector-based `ui.tap` / `ui.inputText` no longer look only at `rootInActiveWindow`, but now prioritize accessibility windows from the current app package before falling back to system windows. After revalidating on-device against Feishu `com.ss.android.lark`, `ui.launchApp` + `ui.tap(text="更多")` now leaves `ui.state.visibleText` consistently showing the bottom-layer entries `编辑`, `视频会议`, and `任务` instead of being completely overwhelmed by status-bar / system-overlay text; however, deeper module entries such as `考勤` are still not yet a stable replayable path.
