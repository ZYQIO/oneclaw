...
- 2026 年 3 月 29 日稍后又补了一层 `ui.launchApp` 真机兼容性：当前 OPPO / ColorOS 手机上，`com.coloros.calculator` 与 `com.coloros.filemanager` 明明可由 launcher 解析，却会被旧实现误报成 `APP_NOT_INSTALLED`。现在 `AndroidManifest.xml` 已声明 `MAIN/LAUNCHER` `queries`，而 `UiAutomationHandler` 也会在 `getLaunchIntentForPackage` 失手时回退到显式 launcher resolve；定向单测 `UiAutomationHandlerLaunchAppTest` 已通过，这两个包都重新回到 `launched=true`，其中 Calculator 还继续通过了 4 秒 cross-app probe。Later on March 29, 2026 we added another layer of real-device compatibility to `ui.launchApp`: on the current OPPO / ColorOS phone, both `com.coloros.calculator` and `com.coloros.filemanager` were launcher-resolvable yet the old implementation still misreported them as `APP_NOT_INSTALLED`. `AndroidManifest.xml` now declares `MAIN/LAUNCHER` `queries`, and `UiAutomationHandler` now falls back to an explicit launcher resolve when `getLaunchIntentForPackage` misses; the targeted `UiAutomationHandlerLaunchAppTest` now passes, both packages are back to `launched=true`, and Calculator then also passed a 4-second cross-app probe.
...
- 这轮还修掉了 follow-up harness 的一个真实 race：在某些 run 里，`ui.launchApp(com.android.settings)` 返回成功时目标 app 其实还没真正成为 active window，导致后续 `ui.tap` 直接报 `UI_TARGET_MISMATCH`。现在 repo-side probe 会先轮询 `ui.state`，确认 target package 真的置前，再开始 `ui.tap` / `ui.inputText`。This round also fixed a real race in the follow-up harness: in some runs, `ui.launchApp(com.android.settings)` returned success before the target app had actually become the active window, which caused the follow-up `ui.tap` to fail immediately with `UI_TARGET_MISMATCH`. The repo-side probe now polls `ui.state` first and only starts `ui.tap` / `ui.inputText` after the target package is truly foregrounded.
- 最新一轮又把同一条 follow-up harness 扩到可选 `ui.swipe`：现在可以通过 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_SWIPE_*` 环境变量，在 reachability polling 前执行受 `packageName` 守卫的滚动，并把 `swipeVisibleTextBefore`、`swipeVisibleTextAfter`、`swipeVisibleTextChanged` 一起写进 `summary.json`。Latest round also extended that same follow-up harness to optional `ui.swipe`: it can now take an `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_SWIPE_*` env set, execute a `packageName`-guarded scroll before reachability polling, and persist `swipeVisibleTextBefore`, `swipeVisibleTextAfter`, and `swipeVisibleTextChanged` into `summary.json`.
- 在落这条 summary 过程中，还顺手修掉了一个 artifact bug：之前 `summary.json` 里对空字符串直接用了 `jq` `select(length > 0)`，会把整份 JSON 写成 0 字节空文件；现在这些可选字段都显式写成 `null`，所以最新的 tap+input 与 swipe follow-up 复跑都已经重新得到稳定非空的 summary artifact。While landing that summary surface, we also fixed an artifact bug: `summary.json` had previously used `jq` `select(length > 0)` directly on empty strings, which could collapse the whole JSON into a 0-byte file; those optional fields now serialize as explicit `null`, so the latest tap+input plus swipe follow-up reruns both leave behind stable non-empty summary artifacts again.
- 同一天又把 repo-side swipe harness 收成了更稳的 preset：`settings-home-swipe-up` 现在默认用 `0..1` ratio 坐标，先从当前 `ui.state` 的 active-window bounds 解析出真实起止点，再发出 `ui.swipe`；所以它不再绑死某一台设备的绝对像素值。Later the same day, that repo-side swipe harness was tightened into a steadier preset: `settings-home-swipe-up` now defaults to `0..1` ratio coordinates, first resolves the real swipe start/end points from the active-window bounds in `ui.state`, and only then issues `ui.swipe`; it is therefore no longer pinned to one device's absolute pixel values.
- 这条 `settings-home-swipe-up` preset 也已经有真机正证据：在先 `adb shell am force-stop com.android.settings` 把 Settings 拉回首页后，`pnpm android:local-host:ui:cross-app:next` 重新跑出了 `classification=foregrounded_host_reachable`、`swipeCoordinateMode=ratio`、`swipeOk=true` 和 `swipeVisibleTextChanged=true`。That `settings-home-swipe-up` preset now also has positive real-device proof: after first resetting Settings back to the homepage with `adb shell am force-stop com.android.settings`, `pnpm android:local-host:ui:cross-app:next` reran with `classification=foregrounded_host_reachable`, `swipeCoordinateMode=ratio`, `swipeOk=true`, and `swipeVisibleTextChanged=true`.
- 继续沿着同一条 Settings state-drift 边界，repo-side probe 现在还支持 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`：它会在 `ui.launchApp` 前先 opt-in `force-stop` 目标 app，给 follow-up harness 一个干净起点，但默认仍然关闭，不会静默抹掉目标 app 状态。Continuing on that same Settings state-drift boundary, the repo-side probe now also supports `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`: it performs one opt-in `force-stop` against the target app before `ui.launchApp`, giving the follow-up harness a clean starting surface while still staying off by default so it does not silently erase target-app state.
- 这条 opt-in reset helper 也已经有真机正证据：先故意跑一轮 `settings-search-input` 把 Settings 留在搜索态，再用 `settings-home-swipe-up` 加 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` 复跑，仍然拿到了 `classification=foregrounded_host_reachable`、`targetResetApplied=true`、`swipeCoordinateMode=ratio` 和 `swipeVisibleTextChanged=true`。That opt-in reset helper now also has positive real-device proof: after intentionally leaving Settings on its search surface via one `settings-search-input` run, rerunning with `settings-home-swipe-up` plus `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true` still produced `classification=foregrounded_host_reachable`, `targetResetApplied=true`, `swipeCoordinateMode=ratio`, and `swipeVisibleTextChanged=true`.
- 为了让这条复跑面不再因为 wrapper 丢配置而退回“手工重拼命令”，`apps/android/scripts/local-host-ui-cross-app-next.sh` 现在也会把当前 wrapper 级显式 env override 一起写回 `recommendedCommand`。因此 `next-summary.json` 里的命令已经足够直接复跑 reset helper、自定义 package 和 sweep windows 这类路径。To prevent this replay surface from falling back to "manually rebuild the command" whenever the wrapper dropped configuration, `apps/android/scripts/local-host-ui-cross-app-next.sh` now also writes the current wrapper-level explicit env overrides back into `recommendedCommand`. As a result, the command in `next-summary.json` is now sufficient to replay paths such as the reset helper, a custom package, or sweep windows directly.
- 这条 replay 能力现在也已经下沉到底层 `probe` / `sweep`：`cross_app.rerun_hint`、sweep `recommendedCommand` 与每档窗口自己的 `probeCommand` 都会保留当前 override，所以后续复跑不再强依赖 wrapper 或手工抄参数。That replay capability has now also been pushed down into the lower-level `probe` / `sweep`: `cross_app.rerun_hint`, the sweep `recommendedCommand`, and each window's own `probeCommand` all preserve the current overrides, so future reruns no longer depend on the wrapper or on manual parameter copying.
- 同一天深夜又把 Calculator 收成了第二条非 Settings 的 repo-side follow-up preset：`calculator-home-open-conversion` 默认目标包是 `com.coloros.calculator`，点击稳定资源 ID `com.coloros.calculator:id/item_open_conversion`，并再加一段 post-action `FINAL_WAIT_TEXT=汇率`；最终 `ui.state` 的 `visibleText` 样本也会一并写进 `summary.json`。这让第二个真实 cross-app target 不再只是“能 launch”，而是已经变成一条可复跑的 tap + final-wait + state 基线。Later that night, Calculator was also tightened into the second non-Settings repo-side follow-up preset: `calculator-home-open-conversion` defaults the target package to `com.coloros.calculator`, taps the stable resource ID `com.coloros.calculator:id/item_open_conversion`, and then adds a post-action `FINAL_WAIT_TEXT=汇率`; the final `ui.state` `visibleText` sample is still written into `summary.json` as well. That means the second real cross-app target is no longer just “launchable,” but has become a replayable tap+final-wait+state baseline too.
- 这轮还顺手补了一条 smoke 级 selector 加固：当前设备上的底部导航 `text=Chat` 点击已经不再稳定切页，所以 `pnpm android:local-host:ui` 现已改为优先点 `contentDescription=Chat`，并重新恢复绿灯。This round also added a smoke-level selector hardening: tapping the bottom-nav `text=Chat` label on the current device no longer switches tabs reliably, so `pnpm android:local-host:ui` now prefers `contentDescription=Chat` and is green again.
...
- 同一天深夜又把飞书 `com.ss.android.lark` 的 selector tap 边界继续收紧了：复查后发现 `ui.tap(text="更多")` 在飞书底栏上并不是 selector miss，而是 `ACTION_CLICK` 可能假成功，导致返回 `performed=true` 却没真正开出模块层。现在 selector 型 `ui.tap` 在命中非 clickable 子节点时会优先走真实 `gesture_tap`，而 `ui.state` 的快照预算也从 `60/12` 提到了 `180/24`。真机重新安装 APK 后，`ui.tap(text="更多")` 已直接返回 `strategy=gesture_tap`，再接 `ui.tap(text="假勤")` 会稳定切到带 `打卡` / `申请` / `统计` / `设置` 的假勤页面，因此“飞书更多 -> 假勤”现在已经拿到第一条完整可回放的正证据。Later that night we tightened the Feishu `com.ss.android.lark` selector-tap boundary further: on reinspection, `ui.tap(text="更多")` on Feishu's bottom bar was not a selector miss, but an `ACTION_CLICK` that could report success without actually opening the module layer. Selector-based `ui.tap` now prefers a real `gesture_tap` whenever the match lands on a non-clickable child, and the `ui.state` snapshot budget has also been raised from `60/12` to `180/24`. After reinstalling the APK on-device, `ui.tap(text="更多")` now directly returns `strategy=gesture_tap`, and a follow-up `ui.tap(text="假勤")` reliably advances into the page showing `打卡` / `申请` / `统计` / `设置`, so "Feishu More -> 假勤" now has its first complete replayable positive proof.
- 紧接着又把“手工 `/invoke` 能做，但聊天代理停在半路”的那层差异补掉了：真机事件流表明，手机上的 Codex agent 会把 `更多`、`假勤` 这类 UI 文案塞进 `nodes.query`，而不是 `nodes.text`，并额外带上 schema 默认的 `x=0/y=0`。旧实现因此会把这些轮次耗在无效 UI 请求上；现在 `LocalHostNodesTooling` 已经把 `query` 自动兼容到 `ui_wait_for_text` / `ui_tap` / `ui_input_text` 的 `text`，并在 selector 已存在时忽略零坐标噪声。最新 `/chat/send-wait` 真机复跑里，提示“打开飞书，在更多选项中打开假勤模块页面。完成后只回复已打开。”已经稳定走完 `ui_launch_app -> ui_wait_for_text(query=更多) -> ui_tap(text=更多) -> ui_wait_for_text(query=假勤) -> ui_tap(text=假勤)` 并返回 `已打开`。Immediately after that we also closed the gap between "manual `/invoke` works" and "the chat agent stalls halfway": the on-device event stream showed that Codex on the phone places UI labels like `更多` and `假勤` into `nodes.query` instead of `nodes.text`, while also dragging along schema-default `x=0/y=0`. The old implementation therefore burned tool turns on ineffective UI requests; `LocalHostNodesTooling` now maps `query` into `text` for `ui_wait_for_text` / `ui_tap` / `ui_input_text`, and ignores zero-coordinate noise whenever a selector already exists. In the latest real `/chat/send-wait` rerun, the prompt "打开飞书，在更多选项中打开假勤模块页面。完成后只回复已打开。" now reliably completes the sequence `ui_launch_app -> ui_wait_for_text(query=更多) -> ui_tap(text=更多) -> ui_wait_for_text(query=假勤) -> ui_tap(text=假勤)` and returns `已打开`.
...
- That `ui.swipe` path now also has direct device evidence on the Settings homepage: after a guarded upward swipe, the visible Settings rows moved from `WLAN` / `蓝牙` / `移动网络` / `连接与共享` into lower entries such as `通知与控制中心` / `密码与安全` / `隐私` / `应用` / `电池` while the active package stayed `com.android.settings`. / 这条 `ui.swipe` 路径现在也已经有设置首页的真机直证：在受 `packageName` 守卫的向上滑动之后，可见设置项从 `WLAN` / `蓝牙` / `移动网络` / `连接与共享` 这一组切换到了 `通知与控制中心` / `密码与安全` / `隐私` / `应用` / `电池` 这组更下层的条目，而且活动包名始终保持 `com.android.settings`。
...
The target is to turn the current manual proof into a reusable validation surface, then keep the existing cross-app follow-up paths healthy while deciding only where a higher-level preset or state-reset helper is actually worth it. `ui.launchApp`, `ui.inputText`, and the coordinate-first `ui.swipe` are already on that repo-side follow-up surface now. / 现在的目标是把当前偏手工的证明路径变成可复用验证面，然后在保持现有跨 app follow-up 路径健康的同时，只判断哪里真的值得补更高层的 preset 或状态重置 helper。`ui.launchApp`、`ui.inputText` 和 coordinate-first 的 `ui.swipe` 现在都已经进入这条 repo-side follow-up 验证面了。
...
- Keep turning `ui.launchApp + ui.inputText` into a repeatable real-device path, including immediate follow-up checks while another app is on top. The repo-side entrypoint for that proof already exists as optional follow-up envs on `apps/android/scripts/local-host-ui-cross-app-probe.sh`, and it now also includes optional `ui.swipe` plus before/after `visibleText` capture in `summary.json`; the next job is to keep those existing proof paths green rather than dropping back to ad-hoc `curl`. / 继续把 `ui.launchApp + ui.inputText` 收敛成可重复的真机路径，包括在其他 app 置前时立即做后续检查。对应的 repo-side 入口已经是 `apps/android/scripts/local-host-ui-cross-app-probe.sh` 上那组可选 follow-up 环境变量，而且现在还带上了可选 `ui.swipe` 与 `summary.json` 里的前后 `visibleText` 采样；下一步要做的是把这些现有证明路径维持成绿色，而不是再退回临时 `curl`。
...
- “第二个真实 cross-app target” 这件事现在已经不再是 blocker：`com.coloros.calculator` 不仅已经通过当前 launcher-query fix 恢复成真机可启动，而且已经被 repo-side `calculator-home-open-conversion` preset 收成一条可复跑 follow-up 基线。后续只在确实有新价值时，才继续加第四条 preset。The question of “do we have a second real cross-app target yet” is no longer a blocker now: `com.coloros.calculator` is not only launchable again on-device after the current launcher-query fix, it has already been turned into a replayable follow-up baseline through the repo-side `calculator-home-open-conversion` preset. We should only add a fourth preset later if it delivers genuinely new value.
- For the current OPPO / ColorOS proof, if you want to replay the Settings-homepage swipe after running `settings-search-input`, prefer `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`; if you keep the reset step manual, `adb shell am force-stop com.android.settings` remains equivalent. The default probe path still preserves prior target-app state unless you opt into that helper. / 对当前 OPPO / ColorOS 这条验证，如果你想在跑过 `settings-search-input` 之后继续重放 Settings 首页 swipe，优先用 `OPENCLAW_ANDROID_LOCAL_HOST_UI_CROSS_APP_FORCE_STOP_TARGET_BEFORE_LAUNCH=true`；如果你仍想手工做 reset，`adb shell am force-stop com.android.settings` 仍然等价。默认 probe 路径仍会保留目标 app 先前状态，除非你显式打开这个 helper。
...
- One repeatable cross-app proof that combines `ui.launchApp` and `ui.inputText`, plus one repeatable repo-side `ui.swipe` follow-up proof with non-empty `summary.json` and `visibleText` deltas. / 一条把 `ui.launchApp` 和 `ui.inputText` 串起来的可重复跨 app 证据；再加上一条带非空 `summary.json` 和 `visibleText` 差异的可重复 repo-side `ui.swipe` follow-up 证据。
