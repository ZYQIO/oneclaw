...
- For the current OPPO / ColorOS proof, if you want to replay the Settings-homepage swipe after running `settings-search-input`, first reset the target app with `adb shell am force-stop com.android.settings`; the probe intentionally does not erase prior target-app state on your behalf. / 对当前 OPPO / ColorOS 这条验证，如果你想在跑过 `settings-search-input` 之后继续重放 Settings 首页 swipe，先用 `adb shell am force-stop com.android.settings` 把目标 app 拉回首页；probe 当前是刻意不替你清除目标 app 先前状态的。
