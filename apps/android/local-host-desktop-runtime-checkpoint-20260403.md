...
Last updated / 最后更新: April 10, 2026 / 2026 年 4 月 10 日
...
- The APK now carries a cohesive `desktop/` bundle plus the first allowlisted packaged plugin descriptor in payload `0.17.0`.
...
- The new repeated-replay live active-session proof is both repo-verified and still freshly replayed on-device: on April 10, 2026 the same `PFEM10` phone again reached `classification=process_runtime_active_session_live_proof_captured` on payload `0.17.0`.
- The branch now also has replay-stability, longer-soak, and reinstall-refresh wrappers: on April 10, 2026 `pnpm android:local-host:embedded-runtime-pod:stability -- --json --iterations 3` still finished with `passedIterationCount=3`, `failedIterationCount=0`, `pnpm android:local-host:embedded-runtime-pod:stability -- --json --iterations 3 --restart-app-between-iterations` also passed with `perturbationMode=app_restart_between_iterations`, `perturbationAppliedCount=2`, and `perturbationFailureCount=0`, `pnpm android:local-host:embedded-runtime-pod:soak -- --json` passed with `packageCommand=pnpm android:local-host:embedded-runtime-pod:soak`, `iterationsRequested=5`, `perturbationAppliedCount=4`, and `perturbationFailureCount=0`, and the new `pnpm android:local-host:embedded-runtime-pod:refresh -- --json` wrapper also passed after auto-falling back from failed `pnpm android:install` to `adb install -r -d`.
- This branch still does **not** have full executable desktop parity yet.

## What Is Already Landed / 已落地内容

- The embedded pod payload is now `0.17.0` and contains `runtime/`, `toolkit/`, `browser/`, `workspace/`, and `desktop/` stages.
...
- That same runtime-smoke path now also writes `runtime-smoke-process-model.json`, `runtime-smoke-activation-contract.json`, `runtime-smoke-supervision-contract.json`, `runtime-smoke-observation-contract.json`, `runtime-smoke-recovery-contract.json`, `runtime-smoke-detached-launch-contract.json`, `runtime-smoke-supervisor-loop-contract.json`, `runtime-smoke-active-session-contract.json`, `runtime-smoke-active-session-validation.json`, and `runtime-smoke-active-session-device-proof.json` under desktop-home state, which together form the branch's current process-runtime bootstrap stack.
...
- There is now one bounded verified live active session through the repeated browser-aligned replay lane, but there is still no full executable detached-process parity beyond the current health/restart/process-model/activation/supervision/observation/recovery/detached-launch/supervisor-loop/active-session stack.
...
5. Prefer the stability wrapper when you want to prove repeatability rather than inspect a single doctor pass, and now prefer the formal `soak` wrapper for the next longer hardening lane.
...
- On April 6, 2026, the branch advanced one repo-side slice further again: payload `0.17.0` now passes `pnpm test -- apps/android/runtime-pod/prepare.test.ts apps/android/runtime-pod/sync-assets.test.ts` plus the targeted Android Gradle suite, `runtime-smoke` writes `runtime-smoke-process-model.json`, `runtime-smoke-activation-contract.json`, `runtime-smoke-supervision-contract.json`, `runtime-smoke-observation-contract.json`, `runtime-smoke-recovery-contract.json`, `runtime-smoke-detached-launch-contract.json`, `runtime-smoke-supervisor-loop-contract.json`, `runtime-smoke-active-session-contract.json`, `runtime-smoke-active-session-validation.json`, and `runtime-smoke-active-session-device-proof.json`, and `pod.runtime.describe` now exposes process-model plus activation-contract plus supervision-contract plus observation-contract plus recovery-contract plus detached-launch plus supervisor-loop plus active-session plus active-session-validation plus active-session-device-proof fields.
...
- Later that same day, the same `PFEM10` lane also passed `pnpm android:local-host:embedded-runtime-pod:stability -- --json --iterations 3 --restart-app-between-iterations`, so the captured live-proof classification stayed green even when the wrapper force-stopped and relaunched `ai.openclaw.app` between doctor runs; the aggregated summary reported `perturbationMode=app_restart_between_iterations`, `perturbationAppliedCount=2`, and `perturbationFailureCount=0`.
- On April 10, 2026, the same `PFEM10` lane also passed `pnpm android:local-host:embedded-runtime-pod:soak -- --json`, so the captured live-proof classification stayed green through a five-iteration restart-perturbation soak; the aggregated summary reported `packageCommand=pnpm android:local-host:embedded-runtime-pod:soak`, `iterationsRequested=5`, `perturbationAppliedCount=4`, `perturbationFailureCount=0`, `passedIterationCount=5`, and `failedIterationCount=0`.
- Later that same day, the same `PFEM10` lane also passed `pnpm android:local-host:embedded-runtime-pod:refresh -- --json`, so the reinstall perturbation path now also reconverges through one formal wrapper; the aggregated summary reported `androidSdkRoot=/Users/zouxiaoyi/Library/Android/sdk`, `install.finalMethod=adb_install_debug_apk`, `install.primary.ok=false`, `install.fallback.used=true`, `install.fallback.ok=true`, `token.tokenRedacted=true`, `passedIterationCount=5`, and `failedIterationCount=0`.
...
- Do not treat the new process-model, activation-contract, supervision-contract, observation-contract, recovery-contract, detached-launch-contract, supervisor-loop-contract, active-session-contract, active-session-validation, or active-session-device-proof bootstrap artifacts as proof that a detached desktop process with a live active session is already running inside Android.
