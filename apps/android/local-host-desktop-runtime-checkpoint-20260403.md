...
- The new process-runtime active-session-device-proof bootstrap is both repo-verified and now freshly replayed on-device: on April 6, 2026 the same `PFEM10` phone reached `classification=process_runtime_active_session_device_proof_bootstrapped` on payload `0.17.0`.
...
- There is still no executed detached subprocess or verified live active session beyond the current health/restart/process-model/activation/supervision/observation/recovery/detached-launch/supervisor-loop/active-session/active-session-validation/active-session-device-proof bootstrap artifacts.
...
4. Keep the refreshed `PFEM10` replay boringly repeatable with `classification=process_runtime_active_session_device_proof_bootstrapped`, then use the browser-aligned `runtimeExecuteAfterBrowser` artifact to drive live active-session proof rather than another repo-only bootstrap.
...
- Later on April 6, 2026, the same `PFEM10` device replayed that `0.17.0` build end to end: `pnpm android:local-host:embedded-runtime-pod:doctor -- --json` converged to `classification=process_runtime_active_session_device_proof_bootstrapped`, and the updated browser-lane smoke also reran `runtime-smoke` after browser replay was ready, yielding `runtimeExecuteAfterBrowser.longLivedProcessReady=true`, `processStatus=standby`, `supervisionStatus=active`, `activeSessionStatus=ready`, `activeSessionValidationStatus=pending_device_proof`, and `activeSessionDeviceProofStatus=pending_live_proof`. The remaining gap is therefore live active-session proof itself rather than another device-side bootstrap unknown.
