...
- The branch now also has both replay-stability and longer-soak wrappers: on April 10, 2026 `pnpm android:local-host:embedded-runtime-pod:stability -- --json --iterations 3` still finished with `passedIterationCount=3`, `failedIterationCount=0`, `pnpm android:local-host:embedded-runtime-pod:stability -- --json --iterations 3 --restart-app-between-iterations` also passed with `perturbationMode=app_restart_between_iterations`, `perturbationAppliedCount=2`, and `perturbationFailureCount=0`, and the new `pnpm android:local-host:embedded-runtime-pod:soak -- --json` wrapper passed with `packageCommand=pnpm android:local-host:embedded-runtime-pod:soak`, `iterationsRequested=5`, `perturbationAppliedCount=4`, and `perturbationFailureCount=0`.
